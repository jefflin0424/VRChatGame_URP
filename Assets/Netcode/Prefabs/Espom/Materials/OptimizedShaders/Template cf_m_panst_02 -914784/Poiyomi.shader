Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/c25faa4c7dddcfa48a3704a3cea48ea6"
{
	Properties
	{
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void ApplyAlphaToCoverage(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (float(2) == 1)
				{
					...
					if (float(0) && float(0))
					{
						poiFragData.alpha *= 1 + max(0, CalcMipLevel(poiMesh.uv[0] * float4(0.001953125,0.0009765625,512,1024).zw)) * float(0.25);
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0.025))
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(1,1,1,0).rgb, float(0)), float4(1,1,1,0).a);
					#ifndef POI_SHADOW
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(1,1,1,0.4745098).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(1,1,1,0.4745098).a;
