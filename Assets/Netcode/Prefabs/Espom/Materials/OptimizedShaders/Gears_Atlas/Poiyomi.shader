Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/6a2fc54ed7f479b44b380d65406071e8"
{
	Properties
	{
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void ApplyAlphaToCoverage(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (float(1) == 1)
				{
					...
					if (float(0) && float(0))
					{
						poiFragData.alpha *= 1 + max(0, CalcMipLevel(poiMesh.uv[0] * float4(0.0001220703,0.0001220703,8192,8192).zw)) * float(0.25);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(0))
				{
					mainUV = sharpSample(float4(0.0001220703,0.0001220703,8192,8192), mainUV);
				}
