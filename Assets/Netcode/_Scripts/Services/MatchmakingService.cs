...
public static class MatchmakingService {
    ...
    public static async Task CreateLobbyWithAllocation(LobbyData data) {
        ...
        var _alloation = await RelayService.Instance.CreateAllocationAsync(data.MaxPlayers);
        var joinCode = await RelayService.Instance.GetJoinCodeAsync(_alloation.AllocationId);
        ...
        //Transport.SetHostRelayData(_alloation.RelayServer.IpV4, (ushort)_alloation.RelayServer.Port, _alloation.AllocationIdBytes, _alloation.Key, _alloation.ConnectionData);
        RelayServerData relayServerData = new RelayServerData(_alloation,"dtls");
    ...
    public static async Task JoinLobbyWithAllocation(string lobbyId) {
        ...
        var _allocation = await RelayService.Instance.JoinAllocationAsync(_currentLobby.Data[Constants.JoinKey].Value);

        //Transport.SetClientRelayData(_allocation.RelayServer.IpV4, (ushort)_allocation.RelayServer.Port, _allocation.AllocationIdBytes, _allocation.Key, _allocation.ConnectionData, _allocation.HostConnectionData);
